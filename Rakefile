...
def copy_role_for_spec
  copy_dest = "spec/roles/bbrfkr.kubernetes_setup"
...
def clone_minion_role_for_spec
  ...
  delete_targets = Dir.glob(copy_dest)
  FileUtils.rm_rf(delete_targets)
  system("cd spec/roles && git clone https://github.com/bbrfkr/ansible_role_kubernetes_minion.git bbrfkr.kubernetes_minion")
end
